<div x-show="tab === 'tab1'">
    <form method="post" action="{{ route('cbrm.storePeribadi') }}" enctype="multipart/form-data">
        @csrf
                            ...
                            <div class="grid grid-cols-6 gap-6 mt-1">
                                ...
                                id="spuose_ic_div">
                                    ...
                                    <input id="spouse_ic_no" name="spouse_ic_no"
                                        value="{{ isset(auth()->user()->peribadi->spouse_ic_no) ? auth()->user()->peribadi->spouse_ic_no : '' }}"
                                        class="mt-1 form-input block w-full py-2 px-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:shadow-outline-blue focus:border-blue-300 transition duration-150 ease-in-out sm:text-sm sm:leading-5" />
                                        @error('spouse_ic_no')
                                                    <p class="text-red-500 text-xs italic mt-4">
                                                        {{ $message }}
...
<script>
    $(document).ready(function(){
        $('select[name=tekun_state]').on('change', function () {
            ...
            $.ajax({
                url: "{{ route('cbrm.getCawangan') }}?negeri=" + selected,
                method: 'GET',
